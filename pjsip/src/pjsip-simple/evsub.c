 ...
 */
PJ_DECL(pj_str_t)
pjsipsimple_strerror(pj_status_t statcode,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_init_module(pjsip_endpoint *endpt)
{
	pj_status_t status;
 ...
 */
PJ_DEF(pjsip_evsub *)
pjsip_tsx_get_evsub(pjsip_transaction *tsx)
{
 ...
 */
PJ_DEF(void)
pjsip_evsub_set_mod_data(pjsip_evsub *sub, unsigned mod_id,
						 void *data)
{
 ...
 */
PJ_DEF(void *)
pjsip_evsub_get_mod_data(pjsip_evsub *sub, unsigned mod_id)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_register_pkg(pjsip_module *pkg_mod,
...
pjsip_evsub_get_allow_events_hdr(pjsip_module *m)
{
 ...
 */
PJ_DEF(void)
pjsip_evsub_uas_set_timeout(pjsip_evsub *sub, pj_uint32_t seconds)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_add_ref(pjsip_evsub *sub)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_dec_ref(pjsip_evsub *sub)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_create_uac(pjsip_dialog *dlg,
					   const pjsip_evsub_user *user_cb,
					   const pj_str_t *event,
					   unsigned option,
					   pjsip_evsub **p_evsub)
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_create_uas(pjsip_dialog *dlg,
					   const pjsip_evsub_user *user_cb,
					   pjsip_rx_data *rdata,
					   unsigned option,
					   pjsip_evsub **p_evsub)
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_terminate(pjsip_evsub *sub,
					  pj_bool_t notify)
{
 ...
 */
PJ_DEF(pjsip_evsub_state)
pjsip_evsub_get_state(pjsip_evsub *sub)
{
	return sub->state;
}
 ...
 */
PJ_DEF(const char *)
pjsip_evsub_get_state_name(pjsip_evsub *sub)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_initiate(pjsip_evsub *sub,
					 ...
					 pjsip_tx_data **p_tdata)
{
    ...
    // srv->name = pj_str((char*)"Allow-Events");
    // srv->hvalue = pj_str((char*)"x-lostcalls,presence");
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_add_header(pjsip_evsub *sub,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_accept(pjsip_evsub *sub,
				   pjsip_rx_data *rdata,
				   int st_code,
				   const pjsip_hdr *hdr_list)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_notify(pjsip_evsub *sub,
				   pjsip_evsub_state state,
				   const pj_str_t *state_str,
				   const pj_str_t *reason,
				   pjsip_tx_data **p_tdata)
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_current_notify(pjsip_evsub *sub,
						   pjsip_tx_data **p_tdata)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_evsub_send_request(pjsip_evsub *sub,
						 pjsip_tx_data *tdata)
{
	pj_status_t status;
									   ...
									   pjsip_event *event)
{
		 ...
		 pjsip_evsub_get_state(dlgsub->sub) == PJSIP_EVSUB_STATE_TERMINATED))
	{
		const char *reason_msg =
			(dlgsub == dlgsub_head ? "Subscription Does Not Exist" : "Subscription already terminated");
							 ...
							 pjsip_event *event)
{
		...
		pjsip_method_cmp(&tsx->method, &pjsip_subscribe_method) == 0)
	{
		...
		if (tsx->status_code / 100 == 2)
		{
			...
			if (sub->expires->ivalue != 0)
			{
				unsigned timeout = (sub->expires->ivalue > TIME_UAC_REFRESH) ? sub->expires->ivalue - TIME_UAC_REFRESH : sub->expires->ivalue;
	...
	else if (pjsip_method_cmp(&tsx->method, &pjsip_notify_method) == 0)
	{
			 ...
			 pj_stricmp(&sub_state->sub_state, &STR_PENDING) == 0))
		{
			...
			/* Start UAC refresh timer, only when we're not unsubscribing */
			timeout = (next_refresh > TIME_UAC_REFRESH) ? next_refresh - TIME_UAC_REFRESH : next_refresh;
