...
PJ_DEF(const char *)
pjsua_vid_win_type_name(pjsua_vid_win_type wt)
{
...
pjsua_vid_preview_has_native(pjmedia_vid_dev_index id)
{
	...
	return (pjmedia_vid_dev_get_info(id, &vdi) == PJ_SUCCESS) ? ((vdi.caps & PJMEDIA_VID_DEV_CAP_INPUT_PREVIEW) != 0) : PJ_FALSE;
}
						...
						const pjsua_vid_preview_param *prm)
{
	...
	status = create_vid_win(PJSUA_WND_TYPE_PREVIEW, fmt, rend_id, id,
							...
							(prm->wnd.info.window ? &prm->wnd : NULL), &wid, -1);
	if (status != PJ_SUCCESS)
	{
		PJSUA_UNLOCK();
		pj_log_pop_indent();
...
/*
 * Set video window flutter texture. window should be invisible for best effect :)
 */
PJ_DEF(pj_status_t)
	...
	pj_uint64_t texture_id)
{
	...
	status = pjmedia_vid_dev_stream_set_cap(s,
											PJMEDIA_VID_DEV_CAP_FLUTTER_TEXTURE, &texture_id);
...
static pj_status_t call_set_video_texture(int call_id,
										  pj_uint64_t texture_id)
{
	pjsua_vid_win *w = NULL;
	pjmedia_vid_dev_stream *vid_dev_stream;
	pj_status_t status;
	int i;
	...
	vid_dev_stream = pjmedia_vid_port_get_stream(w->vp_rend);
	pjmedia_vid_dev_stream_set_cap(vid_dev_stream, PJMEDIA_VID_DEV_CAP_FLUTTER_TEXTURE, &texture_id);
	return PJ_SUCCESS;
}
	...
	const pjsua_call_vid_strm_op_param *param)
{
	...
	case PJSUA_CALL_VID_STRM_SEND_KEYFRAME:
		...
		break;
	case PJSUA_CALL_VID_DECODING_STRM_ATTACH_TEXTURE:
		status = call_set_video_texture(call_id, param_.texture_id);
		break;
	default:
 ...
 */
PJ_DEF(pj_status_t)
pjsua_vid_conf_remove_port(pjsua_conf_port_id id)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsua_vid_conf_update_port(pjsua_conf_port_id id)
{
	...
	pjmedia_dir dir)
{
	...
	for (i = 0; i < call->med_cnt; ++i)
	{
		...
		{
			port_id = (dir == PJMEDIA_DIR_ENCODING) ? call->media[i].strm.v.strm_enc_slot : call->media[i].strm.v.strm_dec_slot;
			break;
		}
	}
