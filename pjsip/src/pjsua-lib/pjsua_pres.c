 ...
 * Find buddy.
 */
PJ_DEF(pjsua_buddy_id)
pjsua_buddy_find(const pj_str_t *uri_str)
{
	pj_str_t input;
	pj_pool_t *pool;
	pjsip_uri *uri;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_buddy_add(const pjsua_buddy_config *cfg,
...
static void subscribe_buddy_presence(pjsua_buddy_id buddy_id)
{
	...
	status = pjsip_pres_create_uac(buddy->dlg, &pres_callback,
								   PJSIP_EVSUB_NO_EVENT_ID, &buddy->sub, &event);
	if (status != PJ_SUCCESS)
	{
		buddy->sub = NULL;
...
pj_status_t pjsua_start_mwi(pjsua_acc_id acc_id, pj_bool_t force_renew)
{
	...
	PJ_ASSERT_RETURN(acc_id >= 0 && acc_id < (int)PJ_ARRAY_SIZE(pjsua_var.acc) && pjsua_var.acc[acc_id].valid, PJ_EINVAL);
